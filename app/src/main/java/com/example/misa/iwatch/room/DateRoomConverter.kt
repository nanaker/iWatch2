...
class DateRoomConverter {
    ...
    //convertir la liste des commentaires
    @TypeConverter
    fun stringToComments(data: String?): ArrayList<Comments> {
        if (data == null) {
    ...
    @TypeConverter
    fun stringToComments(someObjects: ArrayList<Comments>): String {
        return gson.toJson(someObjects)
    }

    //convertir la liste des cinémas
    @TypeConverter
    fun stringToCinema(data: String?): ArrayList<Cinema> {
        if (data == null) {
            return EMPTY_LIST as ArrayList<Cinema>
        }

        val listType = object : TypeToken<ArrayList<Cinema>>() {
    ...
    @TypeConverter
    fun stringToCinema(someObjects: ArrayList<Cinema>): String {
        return gson.toJson(someObjects)
    }

    //convertir la liste des associate_Actors
    @TypeConverter
