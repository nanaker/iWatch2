...
class PopularSeriesPageKeyedDataSource(tmdbApi: TMDBApi): BaseDataSource<Series>(tmdbApi){


    override fun loadInitial(params: LoadInitialParams<Int>, callback: LoadInitialCallback<Int, Series>) {
        networkState.postValue(NetworkState.LOADING)
        tmdbApi.getPopularSeries(1).enqueue(object : Callback<TMDBApi.ListingData<Series>> {
    ...
    override fun loadAfter(params: LoadParams<Int>, callback: LoadCallback<Int, Series>) {
        ...
        networkState.postValue (NetworkState.LOADING)
        tmdbApi.getPopularSeries(params.key).enqueue(object : Callback<TMDBApi.ListingData<Series>> {
